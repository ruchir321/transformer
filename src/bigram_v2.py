...
def encode(s: str) -> list[int]:
    return [encode_mapping[t] for t in s]


def decode(tok: list[int]) -> str:
    return "".join([decode_mapping[t] for t in tok])
...
def get_batch(split):
    ...
    ix = torch.randint(
        high=(len(data) - context_window), size=(batch_size,)
    )  # get random indices for context sequences
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        mat_mul = q @ k.transpose(
            -2, -1
        )  # (B, T, head_size) @ (B, head_size, T) --> (B, T, T) Same as N * N (Bishop, Deep Learning)

        scale = mat_mul * C**-0.5
        mask = scale.masked_fill(
            self.tril[:T, :T] == 0, float("-inf")
        )  # optional: used only in decoder
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.proj = nn.Linear(
            n_embd, n_embd
        )  # projection W_o => dim(n_heads * d_v, d_model) (Vaswani et.al) # (n_heads * d_v) = d_model = n_embd
        self.dropout = nn.Dropout(p=dropout)
...
class Feedforward(nn.Module):
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(
                n_embd, 4 * n_embd
            ),  # in Vaswani et. al. d_model = 512 & d_ff = 2048. So FF layer dimension has a multiplier of 4 in the inner layer
            nn.ReLU(),
...
class BigramModel(nn.Module):
    def __init__(self):
        ...
        self.token_embedding_table = nn.Embedding(
            vocab_size, n_embd
        )  # aka input embeddings (vaswani 2017)
        self.positional_encoding_table = nn.Embedding(context_window, n_embd)

        self.blocks = nn.Sequential(
            *[Block(n_embd=n_embd, n_heads=n_heads) for _ in range(n_layers)]
        )

        # multi-attention heads
        self.sa_heads = MultiHeadAttention(
            num_heads=n_heads, head_size=n_embd // n_heads
        )  # d_v = d_model / n_heads # number of computations is same as single, larger self-attention head
    ...
    def forward(self, idx, targets=None):
        B, T = idx.shape
        token_embd = self.token_embedding_table(
            idx
        )  # (B, T, C=n_embd=d_model) # x tokens are used as indices of embedding table
        pos_enc = self.positional_encoding_table(
...
)  # start with token 0 (i.e. \u character)
